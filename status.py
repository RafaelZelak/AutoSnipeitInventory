...
# Função para obter todos os status existentes no sistema
def listar_status():
    ...
    if response.status_code == 200:
        data = response.json()
        print("Status disponíveis no sistema:")
        for status in data['rows']:
            print(f"Nome: {status['name']} (ID: {status['id']})")
    else:
        print(f"Erro ao buscar status: {response.status_code}, {response.text}")
